...
\begin{enumerate}
%\item \label{item:ant_cluster} M. H. AlFraihi, K. A. Wortman, \emph{Improving Ant Colony Clustering with Genetic Algorithms}.
%\item \label{item:dcel} H. Donze, A. Hsu, D. Garcia, K. A. Wortman, \emph{Line Segment Arrangements in the GPU Model}.
\item \label{item:dsel} A. Hsu, K. A. Wortman, \emph{FCL: A Functional Domain-specific Embedded Language for GPU Programming}
%\item \label{item:fmtree_journal} J. Clay and K. A. Wortman, \emph{A Durable Flash Memory Search Tree.} Preliminary version listed as P-\ref{item:fmtree}.
%\item \label{item:pure_graphs} K. A. Wortman, \emph{Minimalist Pure-Functional Graph Representations}.
\end{enumerate}
...
\textbf{To Appear}
\begin{enumerate}
\item \label{item:oarpdsf_journal} D. Eppstein and K. A. Wortman, \emph{Optimal Angular Resolution for Face-Symmetric Drawings}.  Accepted to Journal of Graph Algorithms and Applications (JGAA).
\end{enumerate}
}
